...
public class DvdController {
    ...
    public String displayDvdList(Map<String, Object> model) {
      ...
      @RequestMapping(value = {"/displayAvgAge"}, method = RequestMethod.GET)
    public String displayAvgAge(Map<String, Object> model) {
        long avgAge = dao.getAverageAge();
        model.put("avgAge", avgAge);
        return "displayAvgAge";
    }
    ...
    public String deleteDvd(@RequestParam("dvdId") int id) {
        dao.removeDvd(id);
        return "redirect:displayDvdList";  //go back to browser, have it resubmit request to server for dvdList

    }

    @RequestMapping(value = "/updateDvd", method = RequestMethod.POST)
    public String updateDvd(HttpServletRequest req, HttpServletResponse res) {  //redirect to dvd List to get udpadted version after editing
        Dvd d = new Dvd();

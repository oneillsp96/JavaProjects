...
//    }
    ...
    public Map<String, List<Dvd>> getByDirectorGroupByRating(String director) {
      ...
      //iterate through dList (which is filled from database)
        for (int i = 0; i < dList.size(); i++) {
            Dvd d = dList.get(i);
            //if director in database equals director passed in...
            if (d.getDirector().equalsIgnoreCase(director)) {
                //on first loop, just create new list
                if (dMap.get(d.getMpaaRating()) == null) {
                    d2List = new ArrayList<>();
                    //if not first loop, get rating from map
                } else {
                    d2List = dMap.get(d.getMpaaRating());

                }
                //add matching DVD from database(dList) to d2List
                d2List.add(d);
                //put d2List into dMap with rating as the key
...
//    }
   ...
   // Java 7 version   --need to get all dvd's, add up ages and divide by total number of dvd's (dList.size())
    @Override
    public long getAverageAge() {
         long ageSum=0;

        List<Dvd> dList = new ArrayList<>();
        //put all dvd's in dList
        dList = jdbcTemplate.query(SQL_GET_ALL_DVDS, new DvdMapper());
        for (int i = 0; i < dList.size(); i++) {
            Dvd d = dList.get(i);
            //add age of each DVD to ageSum - need to do this without resetting
          
                   ageSum+= d.getAge();
      }
        
        long avgAge= (ageSum/ dList.size());
